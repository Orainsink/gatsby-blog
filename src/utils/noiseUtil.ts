...
Marsaglia.createRandomized = function() {
	const now = new Date();
	return new Marsaglia((now.getTime() / 60000) & 0xffffffff, now.getTime() & 0xffffffff);
};

// Noise functions and helpers
function PerlinNoise(seed:number|undefined) {
	let rnd = seed !== undefined ? new Marsaglia(seed as number) : Marsaglia.createRandomized();
	let i, j;
