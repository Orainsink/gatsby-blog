...
`enum`是 typescript 提供的枚举数据类型，是js中没有的概念，而typescript是js的超集，换言之如果你只把typescript当js用，那可能永远不会用到`enum`。大多前端都是从 js 过渡到ts的，导致`enum`很少人使用，很多人也不推荐使用。
...
在类型定义的地方，有暖心的备注。但是在这个备注在不起眼的地方。如果没有备注，或者备注的在另一个文件，那维护的人看到这段代码的时候就很尴尬了。
