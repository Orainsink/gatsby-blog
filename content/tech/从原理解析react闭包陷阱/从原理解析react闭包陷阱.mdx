...
闭包的实现依赖于函数对象的[隐藏属性](https://tc39.es/ecma262/#sec-ecmascript-function-objects)`[[Environment]]`, 这个属性储存了函数声明时的词法环境的引用地址，词法环境是一个无法读取和修改的对象，它的 outer 属性又指向外部词法环境的引用，我们可以通过编译器(V8)实现的`[[Scopes]]`作用域链属性中的`Closure`来观察。
