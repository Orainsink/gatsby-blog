...
当 React 应用补水时，它假设 DOM 结构会固定不变。

当 React 应用第一次在客户端上运行时，它通过挂载你的所有组件，建立起 DOM 应该是什么样子的切片。然后，它眯着眼睛看页面上已有的 DOM 节点，并试图将两者匹配在一起。它不是在玩 "大家来找茬 "的游戏，它只是想把两个节点拼接在一起，以便将来的更新能够正确处理。

服务器端渲染这些不同的东西的过程，我们就像黑进整个系统的黑客。我们打算在服务器上渲染一个东西，但却告诉 React 让他在客户端上期待另一件事。
...
有点值得注意的是，React 有时候还是可以处理这种情况的。你可能自己也做过，而且还能逃过一劫。但你这是在玩火。补水过程的优化是为了 ⚡️ 快速 ⚡️，而不是为了捕捉和修复不匹配。

### gatsby 独有的一个问题

React 团队知道补水不匹配可能会导致时髦的问题，他们已经确保用控制台信息来强调不匹配：

![ssr2](ssr2.png)

不幸的是，Gatsby 只有在为生产构建时才会使用服务器端的渲染 API。而且由于 React 警告一般只在开发过程中发生，这意味着当使用 Gatsby😱 构建时，这些警告永远不会被显示出来。

这是一种权衡。通过在开发中选择不使用服务器端渲染，Gatsby 正在优化一个短暂的反馈循环。能够快速看到你所做的更改是[非常非常重要的](https://vimeo.com/36579366)。Gatsby 优先考虑的是速度，而不是准确性。

这是个很重要的问题，不过；在[一个公开的问题](https://github.com/gatsbyjs/gatsby/issues/17914)中，人们正在倡导改变，我们可能会开始看到水合警告。
...
与我们之前的解决方案不同的是：`useEffect`只有在组件挂载后才会启动。当 React 应用在补水过程中采用 DOM 时，`useEffect`还没有被调用，因此我们满足了 React 的期望。
...
<center>在初始渲染时，会显示一个空白点。挂载后，重新渲染会以真实状态更新它。</center>
...
不过他们不能做到有有效期。在那个时候，制造商根本不知道有效期应该是什么；装满这些盒子的麦片可能还不存在呢！所以他们印上一个空的蓝色长方形。所以他们就印了一个空的蓝色长方形来代替。很久以后，当麦片生产出来并注入盒子后，他们就可以在白色的有效期上盖章，然后打包发货。

两段式渲染也是同样的想法。第一遍，在编译时，产生所有的静态非个人内容，并在动态内容的地方留下漏洞。然后，在React应用挂载到用户的设备上后，第二次传递将所有依赖于客户端状态的动态位盖章。
...
有了这一招，我就能解决我的渲染问题了。这一天被拯救了! 

## 思维模型

虽然整洁，但抽象的东西并不是本教程最重要的部分。关键的一点是心理模型。

在Gatsby应用程序中工作时，我发现用两段式渲染的方式思考真的很有帮助。第一次渲染发生在编译时，提前了很久，为页面奠定了基础，填入了所有用户通用的内容。然后，在很久以后，第二次渲染将填入因人而异的状态位。

Gatsby有时被称为 "静态网站生成器"，但这个名字感觉很有局限性--你可以用Gatsby构建丰富的、动态的Web应用！它确实需要一点特别的考虑。它确实需要一点特殊的考虑，但有了正确的心理模型，任何事情都有可能发生💫。